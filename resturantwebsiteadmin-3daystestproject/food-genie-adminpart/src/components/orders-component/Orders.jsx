...
const Orders = () => {
  ...
  const firebaseOrdersUrl = `${FirebaseDB_URL}/customerOrders.json`;
  const firebaseUsersUrl = `${FirebaseDB_URL}/userProfiles.json`;
  ...
  const updateOrderStatus = async (userId, orderId, status) => {
    try {
      await axios.patch(`${FirebaseDB_URL}/customerOrders/${userId}/${orderId}.json`, {
        status,
      });
