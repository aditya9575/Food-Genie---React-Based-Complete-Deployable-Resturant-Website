...
let firebaseRealtimeDB_Url = import.meta.env.VITE_firebaseRealtimeDb_Url;
...
const GlobalContextProvider = ({ children }) => {
  ...
  const fetchCartItems = async (userId) => {
    try {
      const response = await axios.get(`${firebaseRealtimeDB_Url}/userCartItems/${userId}.json`);
  ...
  const updateCartInFirebase = async (updatedCart) => {
    const promises = updatedCart.map(item =>
      axios.put(`${firebaseRealtimeDB_Url}/userCartItems/${userId}/${item.firebaseId}.json`, { ...item })
    );
  ...
  const addToCart = async (item) => {
    try {
      ...
      if (existingItemIndex !== -1) {
        ...
        const response = await axios.post(`${firebaseRealtimeDB_Url}/userCartItems/${userId}.json`, newItem);
  ...
  const deleteFromCart = async (itemId) => {
    try {
      ...
      if (itemToDelete && itemToDelete.firebaseId) {
        ...
        await axios.delete(`${firebaseRealtimeDB_Url}/userCartItems/${userId}/${itemToDelete.firebaseId}.json`);
  ...
  const clearCart = async () => {
    try {
      ...
      await axios.delete(`${firebaseRealtimeDB_Url}/userCartItems/${userId}.json`);
  ...
  const placeOrder = async () => {
    try {
      ...
      await axios.post(`${firebaseRealtimeDB_Url}/customerOrders/${userId}.json`, order);
  ...
  return (
    <UserContext.Provider value={{
      authenticated, setAuthenticated, cart, addToCart, deleteFromCart, userId, calculateTotalAmount
